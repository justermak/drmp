...
class PlanningVisualizer:
    ...
    def render_robot_trajectories(
        ...
        **kwargs,
        ...
        if trajs is not None:
            _, trajs_coll_idxs, _, trajs_free_idxs, _ = (
                self.task.get_trajs_collision_and_free(trajs, return_indices=True)
            )
            kwargs["colors"] = []
            for i in range(len(trajs_coll_idxs) + len(trajs_free_idxs)):
                kwargs["colors"].append(
                    self.colors["collision"]
                    if i in trajs_coll_idxs
        ...
        def animate_fn(i):
            ...
            for q in qs:
                self.robot.render(
                    ax,
                    q=q,
                    color=self.colors_robot["collision"]
                    if self.task.compute_collision(q, margin=0.0)
                    else self.colors_robot["free"],
                    arrow_length=0.1,
                    arrow_alpha=0.5,
                    arrow_linewidth=1.0,
                    cmap=self.cmaps["collision"]
                    if self.task.compute_collision(q, margin=0.0)
                    else self.cmaps["free"],
                    **kwargs,
                )
    ...
    def plot_joint_space_state_trajectories(
        ...
        trajs=None,
        traj_best=None,
        pos_start_state=None,
        pos_goal_state=None,
        vel_start_state=None,
        vel_goal_state=None,
        ...
        **kwargs,
        ...
        for i, ax in enumerate(axs):
            for trajs_filtered, color in zip(
                [
                    (trajs_coll_pos_np, trajs_coll_vel_np),
                    (trajs_free_pos_np, trajs_free_vel_np),
                ],
                ["black", "orange"],
            ):
                ...
                for j, trajs_filtered_ in enumerate(trajs_filtered):
                    if trajs_filtered_.size > 0:
                        timesteps_ = np.repeat(
                            timesteps, trajs_filtered_.shape[0], axis=0
                        )
                        plot_multiline(
                            ax[j],
                            timesteps_,
            ...
            if traj_best is not None:
                ...
                plot_multiline(
                    ax[0],
                    timesteps,
                    traj_best_pos_np[..., i].reshape(1, -1),
                    color="blue",
                    **kwargs,
                )
                plot_multiline(
                    ax[1],
                    timesteps,
                    traj_best_vel_np[..., i].reshape(1, -1),
                    color="blue",
                    **kwargs,
                )
    ...
    def animate_opt_iters_joint_space_state(
        ...
        fig, axs = self.plot_joint_space_state_trajectories(
            trajs=trajs_selection[0], **kwargs
        )
        ...
        def animate_fn(i):
            ...
            self.plot_joint_space_state_trajectories(
                fig=fig, axs=axs, trajs=trajs_selection[i], **kwargs
            )
...
def create_animation_video(
    fig, animate_fn, anim_time=5, n_frames=100, video_filepath="video.mp4", **kwargs
):
    ...
    print(f"{str_start}...")
    ani.save(
        os.path.join(video_filepath), fps=max(1, int(n_frames / anim_time)), dpi=90
    )
    print(f"...finished {str_start}")
...
def plot_multiline(ax, X, Y, color="blue", linestyle="solid", **kwargs):
    ...
    line_segments = mcoll.LineCollection(
        segments, colors=[color] * len(segments), linestyle=linestyle
    )
