...
class TrajectoryDatasetBase(Dataset, abc.ABC):
    def __init__(
        ...
        **kwargs,
        ...
        self.metadata = load_params_from_yaml(
            os.path.join(self.base_dir, "0", "metadata.yaml")
        )
        ...
        env_class = getattr(
            environments,
            self.metadata["env_id"] + "ExtraObjects"
            if use_extra_objects
            else self.metadata["env_id"],
        )
        ...
        self.task = PlanningTask(
            env=self.env, robot=self.robot, tensor_args=tensor_args, **self.args
        )
    ...
    def load_trajectories(self):
        ...
        task = torch.cat(
            (trajs_free_pos[..., 0, :], trajs_free_pos[..., -1, :]), dim=-1
        )
    ...
    def get_hard_conditions(self, traj, horizon=None, normalize=False):
        ...
        if self.include_velocity:
            ...
            start_state = torch.cat(
                (start_state_pos, torch.zeros_like(start_state_pos)), dim=-1
            )
            goal_state = torch.cat(
                (goal_state_pos, torch.zeros_like(goal_state_pos)), dim=-1
            )
        else:
            start_state = start_state_pos
            goal_state = goal_state_pos
