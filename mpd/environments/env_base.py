...
from mpd.environments.grid_map_sdf import GridMapSDF
from mpd.environments.primitives import ObjectField, MultiSphereField, MultiBoxField
from mpd.utils.torch_timer import TimerCUDA
from mpd.utils.torch_utils import to_numpy
from mpd.config import DEFAULT_TENSOR_ARGS
from mpd.visualizers.planning_visualizer import create_fig_and_axes
...
class EnvBase(ABC):
        ...
        **kwargs,
        ...
        if obj_fixed_list is not None:
            for obj in obj_fixed_list:
                assert isinstance(obj, ObjectField), (
                    "Objects must be instances of ObjectField class"
                )
    ...
    def render_sdf(self, ax=None, fig=None):
        # draw sdf
        xs = torch.linspace(
            self.limits_np[0][0], self.limits_np[1][0], steps=200, **self.tensor_args
        )
        ys = torch.linspace(
            self.limits_np[0][1], self.limits_np[1][1], steps=200, **self.tensor_args
        )
        if self.dim == 3:
            zs = torch.linspace(
                self.limits_np[0][2],
        ...
        if self.dim == 3:
            ...
            stacked_tensors = torch.stack((X_flat, Y_flat, Z_flat), dim=-1).view(
                -1, 1, self.dim
            )
        else:
            stacked_tensors = torch.stack((X_flat, Y_flat), dim=-1).view(
                -1, 1, self.dim
            )
        ...
        if self.dim == 3:
            ...
            random_idxs = np.random.choice(
                np.arange(len(idxs_sdf[0])), size=5000, replace=False
            )  # for downsampling
            idxs = (
                idxs_sdf[0][random_idxs],
                idxs_sdf[1][random_idxs],
                idxs_sdf[2][random_idxs],
            )
    ...
    def render_grad_sdf(self, ax=None, fig=None):
        ...
        xs = torch.linspace(
            self.limits_np[0][0], self.limits_np[1][0], steps=40, **self.tensor_args
        )
        ys = torch.linspace(
            self.limits_np[0][1], self.limits_np[1][1], steps=40, **self.tensor_args
        )
        if self.dim == 3:
            zs = torch.linspace(
                self.limits_np[0][2], self.limits_np[1][2], steps=20, **self.tensor_args
            )
            X, Y, Z = torch.meshgrid(xs, ys, zs, indexing="xy")
        else:
            X, Y = torch.meshgrid(xs, ys, indexing="xy")
