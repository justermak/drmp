...
def avg_ctrl_to_goal(
    ...
    if control_type == "velocity":
        ...
        return ((target[:pos_dim] - state[:pos_dim]) / rollout_steps / dt**2).clamp(
            max=max_ctrl
        )
...
def get_multivar_gaussian_prior(
    ...
    return ControlTrajectoryGaussian(
        rollout_steps, control_dim, mu, Cov, tensor_args=tensor_args
    )
...
class ControlTrajectoryPrior(ABC):
    ...
    def sample(
        ...
        """
        U_s = torch.empty(
            num_samples, self.rollout_steps, self.ctrl_dim, **self.tensor_args
        )
        # TODO - remove this for loop
        for i in range(self.ctrl_dim):
            U_s[:, :, i] = (
                self.list_ctrl_dists[i].sample((num_samples,)).to(**self.tensor_args)
            )
        return U_s
        ...
        for i in range(self.ctrl_dim):
            self.list_ctrl_dists.append(
                dist.MultivariateNormal(
                    self.mu[:, i], covariance_matrix=self.Cov[:, :, i]
                )
            )
