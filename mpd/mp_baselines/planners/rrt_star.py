...
from mp_baselines.planners.utils import safe_path, purge_duplicates_from_traj
    ...
    def render(self, ax):
        ...
        if self.parent is not None:
            if ax.name == "3d":
                ...
                ax.plot3D(
                    [x[0], y[0]], [x[1], y[1]], [x[2], y[2]], color="k", linewidth=0.5
                )
            else:
                x, y = self.config.cpu().numpy(), self.parent.config.cpu().numpy()
...
class RRTStar(RRTBase):
    def __init__(
        ...
        self.max_best_cost_iters = (
            max_best_cost_iters if max_best_cost_iters is not None else n_iters
        )
    ...
    def _run_optimization(self, opt_iters, **observation):
        ...
        if (
            self.collision_fn(self.start_state_pos).squeeze()
            or self.collision_fn(self.goal_state_pos).squeeze()
        ):
            return None
        ...
        with TimerCUDA() as t:
            while (t.elapsed < self.max_time) and (iteration < self.n_iters):
                ...
                # Sample new node
                do_goal = goal_n is None and (
                    iteration == 0 or torch.rand(1) < self.goal_prob
                )
                if do_goal:
                ...
                if (
                    ...
                    or iteration % (self.n_iters - 1) == 0
                    or iters_after_first_success == 1
                ):
                    if debug:
                ...
                if (
                    ...
                    and (
                        self.distance_fn(self.start_state_pos, s)
                        + self.distance_fn(s, self.goal_state_pos)
                        >= goal_n.cost
                    )
                ):
                ...
                extended = self.extend_fn(
                    nearest.config, s, max_step=self.step_size, max_dist=self.n_radius
                )
                ...
                if do_goal and (
                    self.distance_fn(new.config, self.goal_state_pos) < eps
                ):
                    goal_n = new
                ...
                if neighbors_idxs.nelement() != 0:
                    neighbors_idxs = np.atleast_1d(
                        neighbors_idxs.squeeze().cpu().numpy()
                    )
                    try:
