...
class MPPI(MPPlanner):
    ...
    def update_controller(self, costs, U_sampled):
        ...
        self._mean.add_(
            self.step_size * (weights * (U_sampled - self._mean.unsqueeze(0))).sum(0)
        )
    ...
    def sample_and_eval(self, **observation):
        ...
        state_trajectories = self.get_state_trajectories_rollout(
            ...
            num_ctrl_samples=self.num_ctrl_samples,
            **observation,
        )
        self.state_trajectories = state_trajectories.clone()
    ...
    def optimize(self, opt_iters=None, **observation):
        ...
        for opt_step in range(opt_iters):
            with torch.no_grad():
                (control_samples, state_trajectories, costs) = self.sample_and_eval(
                    **observation
                )
                self._save_best()
    ...
    def get_state_trajectories_rollout(
        self, controls=None, num_ctrl_samples=None, **observation
    ):
        state_trajectories = torch.empty(
