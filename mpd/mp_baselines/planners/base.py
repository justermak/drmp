...
class OptimizationPlanner(MPPlanner):
    def __init__(
        ...
        **kwargs,
        ...
        ):  # NOTE(an): if there is no goal, we assume xere is at least one solution
            self.num_goals = 1
        else:
        ...
        if self.pos_only:
            ...
            self.start_state = torch.cat(
                [self.start_state, torch.zeros_like(self.start_state)], dim=-1
            )
            if self.multi_goal_states is not None:
                self.multi_goal_states = torch.cat(
                    [self.multi_goal_states, torch.zeros_like(self.multi_goal_states)],
                    dim=-1,
                )
    ...
    def const_vel_trajectories(
        ...
        mean_vel = (
            multi_goal_states[:, : self.n_dof] - start_state[:, : self.n_dof]
        ) / (self.n_support_points * self.dt)
        for i in range(self.n_support_points):
            state_traj[:, i, : self.n_dof] = start_state[:, : self.n_dof] * (
                self.n_support_points - i - 1
            ) / (self.n_support_points - 1) + multi_goal_states[:, : self.n_dof] * i / (
                self.n_support_points - 1
            )
        state_traj[:, :, self.n_dof :] = mean_vel.unsqueeze(1).repeat(
            1, self.n_support_points, 1
        )
        return state_traj
    ...
    def get_random_trajs(self):
        ...
        particles = self._traj_dist.sample(self.num_particles_per_goal).to(
            **tensor_args
        )
