...
class GPMP2(OptimizationPlanner):
    ...
    def reset(
        ...
        if initial_particle_means is not None:
            ...
            self._particle_means = self._init_dist.sample(
        ...
        for opt_step in range(opt_iters):
            ...
            if self.stop_criteria is not None:
                ...
                if opt_step == 0:
                    ...
                    continue
                if torch.all(
                    torch.abs((costs - costs_previous) / costs) < self.stop_criteria
                ):
                    break
                costs_previous = costs.clone()
    ...
    def _step(self, debug=False, **observation):
        ...
        J_t_J, g = self._get_grad_terms(
            ...
            sparse_computation_block_diag=self.solver_params.get(
        ...
        if not sparse_computation:
            ...
            if not sparse_computation_block_diag:
                ...
                for A_, K_, b_ in zip(A, K, b):
                    A_t_K_sparse = torch.sparse.mm(
                        A_.transpose(-2, -1).to_sparse(), K_.to_sparse()
                    )
                ...
                A_t_block_diag_sparse = torch.block_diag(
                    *A.transpose(-2, -1)
                ).to_sparse()
                K_block_diag_sparse = torch.block_diag(*K).to_sparse()

                A_t_K_sparse = torch.sparse.mm(
                    A_t_block_diag_sparse, K_block_diag_sparse
                )
                ...
                g_sparse = torch.sparse.mm(
                    A_t_K_sparse, torch.block_diag(*b).to_sparse()
                )
                ...
                J_t_J = torch.stack(
                    [
                        J_t_J_block_diag[i * N : (i + 1) * N, i * N : (i + 1) * N]
                        for i in range(B)
                    ]
                )
                g = torch.stack(
                    [
                        g_block_diag[i * N : (i + 1) * N, i].unsqueeze(-1)
                        for i in range(B)
                    ]
                )
