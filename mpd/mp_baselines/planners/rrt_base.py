...
class RRTBase(MPPlanner):
    ...
    def reset(self):
        ...
        n_uniform_samples = self.n_pre_samples - (
            self.pre_samples.shape[0] if self.pre_samples is not None else 0
        )
    ...
    def remove_last_pre_sample(self):
        ...
        if len(self.pre_samples) > 0:
            ...
            self.pre_samples = torch.cat(
                [self.pre_samples[:i], self.pre_samples[i + 1 :]]
            )
    ...
    def random_collision_free(self, **observation):
        ...
        if len(self.pre_samples) > 0:
            ...
            self.pre_samples = self.create_uniform_samples(
                self.n_samples_refill, **observation
            )
            qs = self.get_pre_sample()
        else:
