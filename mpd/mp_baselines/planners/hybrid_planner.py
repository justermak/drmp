...
from mpd.utils.torch_utils import tensor_linspace_v1
from mpd.trajectory.utils import smoothen_trajectory
...
class HybridPlanner(MPPlanner):
    ...
    def optimize(
        self, debug=False, print_times=False, return_iterations=False, **kwargs
    ):
        with TimerCUDA() as t_hybrid:
            ...
            with TimerCUDA() as t_sample_based:
                traj_l = self.sample_based_planner.optimize(
                    refill_samples_buffer=True, debug=debug, **kwargs
                )
            ...
            initial_traj_pos_vel = einops.rearrange(
                initial_traj_pos_vel, "n h d -> 1 n h d"
            )
            ...
            trajs_iters = torch.empty(
                (self.opt_based_planner.opt_iters + 1, *trajs_0.shape),
                **self.tensor_args,
            )
            ...
            with TimerCUDA() as t_opt_based:
                for i in range(self.opt_based_planner.opt_iters):
                    trajs = self.opt_based_planner.optimize(
                        opt_iters=1, debug=debug, **kwargs
                    )
