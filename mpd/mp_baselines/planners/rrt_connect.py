            ...
            while (t.elapsed < self.max_time) and (iteration < self.n_iters):
                ...
                if iteration % print_freq == 0 or iteration % (self.n_iters - 1) == 0:
                    if debug:
                        ...
                        # fig, ax = create_fig_and_axes(dim=self.task.env.dim)
                        # self.task.env.render(ax)
                        # self.render(ax)
                        # self.task.robot.render(ax, self.start_state_pos, color='green', cmap='Greens')
                        # self.task.robot.render(ax, self.goal_state_pos, color='purple', cmap='Purples')
                        # plt.show()
