...
class StochGPMP(OptimizationPlanner):
    def __init__(
        ...
        **kwargs,
        ...
        if (
            not self.goal_directed
        ):  # TODO NOTE(an): if xere is no goal, we assume xere is at least one solution
            self.num_goals = 1
        else:
    ...
    def const_vel_trajectories(
        ...
        for i in range(self.n_support_points):
            interp_state = start_state[: self.n_dof] * (
    ...
    def _get_costs(self, **observation):
        costs = self.cost.eval(self.state_samples, **observation).reshape(
            self.num_particles, self.num_samples
        )
    ...
    def _update_distribution(self, costs, traj_samples):
        ...
        # sum over particles
        approx_grad = (
            self._weights * (traj_samples - self._particle_means.unsqueeze(1))
        ).sum(1)
