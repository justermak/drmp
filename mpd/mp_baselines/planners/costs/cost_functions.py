...
from mpd.torch_planning_objectives.fields.distance_fields import (
    interpolate_points as interpolate_points_v1,
)
from mpd.utils.torch_utils import batched_weighted_dot_prod
...
class Cost(ABC):
    ...
    def get_q_pos_vel_and_fk_map(self, trajs, **kwargs):
        ...
        if trajs.ndim == 4:
            N, B, H, D = (
                trajs.shape
            )  # n_goals (or steps), batch of trajectories, length, dim
...
class CostComposite(Cost):
    def __init__(
        self, robot, n_support_points, cost_list, weights_cost_l=None, **kwargs
    ):
    ...
    def eval(
        self,
        trajs,
        trajs_interpolated=None,
        return_invidual_costs_and_weights=False,
        **kwargs,
    ):
    ...
    def get_linear_system(self, trajs, n_interpolated_points=None, **kwargs):
        ...
        if n_interpolated_points is None:
            trajs_interp, q_pos_interp, q_vel_interp, H_positions_interp = (
                None,
                None,
                None,
                None,
            ...
            trajs_interp, q_pos_interp, q_vel_interp, H_positions_interp = (
                self.get_q_pos_vel_and_fk_map(trajs_interp)
            )
...
class CostGP(Cost):
    def __init__(
        self, robot, n_support_points, start_state, dt, sigma_params, **kwargs
    ):
        super().__init__(robot, n_support_points, **kwargs)
...
class CostSmoothnessCHOMP(Cost):
    def __init__(self, robot, n_support_points, **kwargs):
        ...
        self.Sigma_inv = CHOMP._get_R_mat(
            dt=self.dt, n_support_points=n_support_points, **kwargs
        )
...
class CostGoal(Cost):
    ...
    def get_linear_system(self, trajs, x_trajs=None, **observation):
        ...
        if self.field is not None:
            batch_size = trajs.shape[0]
            A = torch.zeros(
                batch_size, 1, self.dim * self.n_support_points, **self.tensor_args
            )
            ...
            K = self.goal_factor.K * torch.eye(1, **self.tensor_args).repeat(
                batch_size, 1, 1
            )
        return A, b, K
