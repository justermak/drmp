...
class STOMP(OptimizationPlanner):
    ...
    def const_vel_trajectory(
        ...
        if not self.pos_only:
            mean_vel = (goal_state[: self.n_dof] - start_state[: self.n_dof]) / (
                num_steps * self.dt
            )
    ...
    def _sample_and_eval(self, **observation):
        ...
        costs = einops.rearrange(
            costs, "(p s) -> p s", p=self.num_particles, s=self.num_samples
        )
    ...
    def _update_distribution(self, costs, traj_particles):
        ...
        self._weights = self._weights.reshape(
            self.num_particles, self.num_samples, 1, 1
        )
