...
class PointParticleDynamics:
    ...
    def traj_cost(self, X_in, U_in, **observation):
        ...
        pos_cost = (
            torch.square(dX[..., : self.state_dim]) * self._c_weights["pos"]
        ).sum(-1) * discount_seq
        vel_cost = (
            torch.square(dX[..., self.state_dim : self.control_dim])
            * self._c_weights["vel"]
        ).sum(-1) * discount_seq
        control_cost = (torch.square(U) * self._c_weights["ctrl"]).sum(
            -1
        ) * discount_seq
        terminal_cost = (torch.square(dX_final) * self._c_weights["pos_T"]).sum(
            -1
        ) * discount_seq[..., -1]
        ...
        if self.verbose:
            ...
            print(
                "control_cost: {:5.4f}".format(
                    control_cost.mean().detach().cpu().numpy()
                )
            )
            print(
                "terminal_cost: {:5.4f}".format(
                    terminal_cost.mean().detach().cpu().numpy()
                )
            )
            if cost is not None:
                print(
                    "energy_cost: {:5.4f}".format(
                        energy_cost.mean().detach().cpu().numpy()
                    )
                )
            print("")
