...
def smoothen_trajectory(
    ...
    assert not (set_average_velocity and zero_velocity), (
        "Either sets the average velocity or zero velocity"
    )
...
def interpolate_traj_via_points(trajs, num_interpolation=10):
    ...
    if num_interpolation > 0:
        ...
        alpha = torch.linspace(0, 1, num_interpolation + 2).type_as(trajs)[
            1 : num_interpolation + 1
        ]
