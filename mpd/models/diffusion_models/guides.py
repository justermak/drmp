...
from mpd.torch_planning_objectives.fields.distance_fields import interpolate_points
    ...
    def forward(self, x_pos_normalized):
        ...
        with torch.enable_grad():
            ...
            if self.interpolate_trajectories_for_collision:
                ...
                x_interpolated = interpolate_points(
                    x_pos,
            ...
            cost_l, weight_grad_cost_l = self.cost(
                x_pos_vel,
                ...
                if torch.is_tensor(cost):
                    ...
                    if self.use_velocity_from_finite_difference:
                        grad_cost = torch.autograd.grad(
                            [cost.sum()], [x_pos], retain_graph=True
                        )[0]
                    else:
                        grad_cost, grad_cost_velocity = torch.autograd.grad(
                            [cost.sum()], [x_pos, self.velocity]
                        )

                    # clip gradients
                    grad_cost_clipped = self.clip_gradient(grad_cost)
                    if not self.use_velocity_from_finite_difference:
                        grad_cost_velocity_clipped = self.clip_gradient(
                            grad_cost_velocity
                        )
                    ...
                    if not self.use_velocity_from_finite_difference:
                        grad_cost_velocity_clipped[..., 0, :] = 0.0
                        grad_cost_velocity_clipped[..., -1, :] = 0.0
                    ...
                    if not self.use_velocity_from_finite_difference:
                        grad_cost_velocity_clipped_weighted = (
                            weight_grad_cost * grad_cost_velocity_clipped
                        )
...
class GuideSmoothnessGPPrior(GuideBase):
    ...
    def __init__(
        self, n_dofs, n_support_points_des, start_state, dt, cost_sigmas, **kwargs
    ):
        super().__init__(**kwargs)
        # self.cost_prior = CostGP(n_dofs, n_support_points_des, start_state, dt, cost_sigmas, **kwargs)
        self.cost_prior = CostGPTrajectory(
            n_dofs, n_support_points_des, start_state, dt, cost_sigmas, **kwargs
        )
...
class GuideTrajectorySmoothnessFirstOrder(GuideBase):
    ...
    def forward(self, x):
        ...
        return -1.0 * distance  # maximize the negative distance = minimize distance
...
class GuideTrajectoryLastPoint(GuideBase):
    ...
    def forward(self, x):
        ...
        # Equation A8
        objective = -1.0 * l1_dist_goal_state.sum(-1)
        return objective
...
class GuideStochGPMP(GuideBase):
    ...
    def plan_sgpmp(
        self, prior_traj, sgpmp_opt_iters=1, step_size=0.05, return_trajs=False
    ):
        ...
        start_state = torch.cat(
            (self.start_state, torch.zeros(n_dofs, **self.tensor_args))
        )
        multi_goal_states = einops.repeat(
            torch.cat((self.goal_state, torch.zeros(n_dofs, **self.tensor_args))),
            "d -> b d",
            b=b,
        )
        ...
        cost_prior = CostGP(
            n_dofs, n_support_points_des, start_state, dt, cost_sigmas, self.tensor_args
        )
        ...
        if prior_traj is not None:
            ...
            prior_traj = einops.repeat(
                prior_traj, "b h d -> b n h d", n=num_particles_per_goal
            )
            # Add velocities to state
            initial_particle_means = torch.cat(
                (prior_traj, torch.zeros_like(prior_traj)), dim=-1
            )
        ...
        controls, _, trajectories, trajectory_means, weights = (
            sgpmp_planner.get_recent_samples()
        )
