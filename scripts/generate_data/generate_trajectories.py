...
from mpd.mp_baselines.planners.gpmp2 import GPMP2
from mpd.mp_baselines.planners.hybrid_planner import HybridPlanner
from mpd.mp_baselines.planners.multi_sample_based_planner import MultiSampleBasedPlanner
from mpd.mp_baselines.planners.rrt_connect import RRTConnect
from mpd import environments, robots
from mpd.tasks.tasks import PlanningTask
from mpd.visualizers.planning_visualizer import PlanningVisualizer
...
def generate_collision_free_trajectories(
    ...
    for _ in range(n_tries):
        ...
        if (
            torch.linalg.norm(start_state_pos - goal_state_pos)
    ...
    if start_state_pos is None or goal_state_pos is None:
        raise ValueError(
            f"No collision free configuration was found\n"
    ...
    planner = HybridPlanner(
        sample_based_planner, opt_based_planner, tensor_args=tensor_args
    )

    # Optimize
    trajs_iters = planner.optimize(
        debug=debug, print_times=True, return_iterations=True
    )
    ...
    print(f"----------------STATISTICS----------------")
    print(
        f"percentage free trajs: {task.compute_fraction_free_trajs(trajs_last_iter) * 100:.2f}"
    )
    print(
        f"percentage collision intensity {task.compute_collision_intensity_trajs(trajs_last_iter) * 100:.2f}"
    )
    print(f"success {task.compute_success_free_trajs(trajs_last_iter)}")

    # save
    torch.cuda.empty_cache()
    trajs_last_iter_coll, trajs_last_iter_free = task.get_trajs_collision_and_free(
        trajs_last_iter
    )
    ...
    trajs_iters_coll, trajs_iters_free = task.get_trajs_collision_and_free(
        trajs_iters[-1]
    )
    results_data_dict = {
        "duration": duration,
        ...
        "trajs_iters_coll": trajs_iters_coll.unsqueeze(0)
        if trajs_iters_coll is not None
        else None,
        "trajs_iters_free": trajs_iters_free.unsqueeze(0)
    ...
    num_trajectories_coll, num_trajectories_free = (
        len(trajs_last_iter_coll),
        len(trajs_last_iter_free),
    )
