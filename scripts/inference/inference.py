...
from mpd.mp_baselines.planners.costs.cost_functions import (
    CostCollision,
    CostComposite,
    CostGPTrajectory,
)
...
from mpd.utils.torch_utils import get_torch_device, freeze_torch_model_params
from mpd.trajectory.metrics import (
    compute_smoothness,
    compute_path_length,
    compute_variance_waypoints,
)
    ...
    **kwargs,
    ...
    for _ in range(n_tries):
        ...
        if (
            ...
            > dataset.threshold_start_goal_pos
        ):
            break
    ...
    cost_smoothness_l = [
        CostGPTrajectory(
            robot, n_support_points, dt, sigma_gp=1.0, tensor_args=tensor_args
        )
    ]
    weights_grad_cost_l.append(weight_grad_cost_smoothness)

    ####### Cost composition
    cost_func_list = [*cost_collision_l, *cost_smoothness_l]
    ...
    if run_prior_then_guidance:
        n_post_diffusion_guide_steps = (
            t_start_guide + n_diffusion_steps_without_noise
        ) * n_guide_steps
        with TimerCUDA() as timer_post_model_sample_guide:
            ...
            chain = einops.rearrange(
                chain, "b post_diff_guide_steps h d -> post_diff_guide_steps b h d"
            )
            trajs_normalized_iters = torch.cat((trajs_normalized_iters, chain))
        print(
            f"t_post_diffusion_guide: {timer_post_model_sample_guide.elapsed:.3f} sec"
        )
    ...
    (
        ...
        _,
    ) = task.get_trajs_collision_and_free(trajs_final, return_indices=True)
    ...
    if trajs_final_free is not None:
        ...
        print(
            f"cost smoothness: {cost_smoothness.mean():.4f}, {cost_smoothness.std():.4f}"
        )

        cost_path_length = compute_path_length(trajs_final_free, robot)
        print(
            f"cost path length: {cost_path_length.mean():.4f}, {cost_path_length.std():.4f}"
        )
        ...
        variance_waypoint_trajs_final_free = compute_variance_waypoints(
    ...
    if render:
        ...
        planner_visualizer.animate_opt_iters_joint_space_state(
            ...
            video_filepath=os.path.join(
                results_dir, f"{base_file_name}-joint-space-opt-iters.mp4"
            ),
        ...
        planner_visualizer.animate_opt_iters_robots(
            ...
            traj_best=traj_final_free_best,
            video_filepath=os.path.join(
                results_dir, f"{base_file_name}-traj-opt-iters.mp4"
            ),
            n_frames=max((2, len(trajs_iters))),
            anim_time=5,
        )
        ...
        planner_visualizer.animate_robot_trajectories(
            trajs=pos_trajs_iters[-1],
            start_state=start_state_pos,
            goal_state=goal_state_pos,
            plot_trajs=True,
            video_filepath=os.path.join(
                results_dir, f"{base_file_name}-robot-traj.mp4"
            ),
