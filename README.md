...
gpmp2 - GPMP2. Start from a straight line trajectory connecting start and finish.

grad - simple optimization algorithm based on gradient descent. Start from a straight line trajectory connecting start and finish.

grad-splines - the same algorithm but ran in the space of BSpline control-points. Start from a straight line trajectory 
connecting start and finish, represented as BSpline control points.

rrtconnect - RRT-Connect

rrtconnect-spline - use positions generated by RRT-Connect as control points for a BSpline 

rrtconnect-gpmp2 - run RRT-Connect, then smoothen trajectory using bspline, pick N points evenly on the bspline and run GPMP2 on them

rrtconnect-grad - same but run Gradient Descent based optimization

rrtconnect-grad-splines - after smoothening trajectories generated by RRT-Connect with a BSpline, optimize the resulting control points using Gradient Descent

mpd - model's from paper MPD

mpd-reimpl - model trained similarly to MPD but with slightly different architecture and different data

mpd-splines-reimpl - model trained similarly to MPD-Splines but with slightly different architecture and different data

drmp (ours) - our model that is similar to MPD-Splines, but optimized for efficient one-step generation
