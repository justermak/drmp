...
class GuideTrajectories(nn.Module):
    ...
    def forward(self, trajectories_normalized: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        if trajectories_normalized.shape[-1] == N_DIM:
            t_normalized = torch.cat([t_normalized, torch.zeros_like(t_normalized)], dim=-1)
        ...
        with torch.enable_grad():
            ...
            cost = self.cost(trajectories=trajectories_dense, n_interpolate=self.n_interpolate).sum()

            grad = torch.autograd.grad(cost, t)[0]
