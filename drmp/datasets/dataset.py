...
class TrajectoryDataset(Dataset):
    def __init__(
        ...
        self.generating_robot: Robot = Robot(
            margin=robot_margin + cutoff_margin,
            dt=duration / (n_support_points - 1),
            tensor_args=tensor_args,
        )
    ...
    def _create_planner(
        ...
        sample_based_planner = RRTConnect(
            ...
            n_samples=rrt_connect_default_params_env["n_samples"],
        )
        ...
        optimization_based_planner = GPMP2(
            ...
            sigma_collision=gpmp_default_params_env["sigma_collision"],
    ...
    def generate_trajectories_for_task(
        ...
        trajs_collision, trajs_free, points_collision_mask = (
            self.env.get_trajs_collision_and_free(robot=self.robot, trajs=trajs)
        )

        print(f"--------- STATISTICS ---------")
        print(f"total trajectories: {len(trajs)}")
        print(
            f"free fraction: {compute_free_fraction(trajs_free, trajs_collision) * 100:.2f}"
        )
        print(
    ...
    def load_train_val_split(
        ...
        train_idx = torch.load(os.path.join(self.dataset_dir, f"train_idx.pt"))
        val_idx = torch.load(os.path.join(self.dataset_dir, f"val_idx.pt"))
