...
from drmp.planning.planners.parallel_sample_based_planner import ParallelSampleBasedPlanner
...
class HybridPlanner():
    ...
    def optimize(self, sample_iters: int, opt_iters: int, print_freq: int = 100, debug: bool = False, **kwargs) -> torch.Tensor:
        with TimerCUDA() as t_hybrid:
            with TimerCUDA() as t_sample_based:
                trajs = self.sample_based_planner.optimize(
                    sample_iters=sample_iters, print_freq=print_freq, debug=debug, **kwargs
                )
            if debug:
                print(
            ...
            trajs_smooth = [smoothen_trajectory(
                traj,
            ...
            with TimerCUDA() as t_opt_based:
                self.opt_based_planner.reset(initial_particle_means=init_trajs)
                trajs = self.opt_based_planner.optimize(opt_iters=opt_iters, print_freq=print_freq, debug=debug)
            if debug:
                print(
