...
def compute_waypoints_variance(trajs: torch.Tensor, robot: Robot):
    ...
    if trajs.shape[0] == 0:
        return torch.tensor(0.0)
    trajs_pos = (
        robot.get_position(trajs).permute(1, 0, 2)
        if robot is not None
        else trajs.permute(1, 0, 2)
    )
    mean = torch.mean(trajs_pos, dim=-2, keepdim=True)
    cov = (
        (trajs_pos.unsqueeze(-2) - mean.unsqueeze(-2))
        * (trajs_pos.unsqueeze(-1) - mean.unsqueeze(-1))
    ).sum(dim=-3) / (trajs_pos.shape[-2] - 1)
