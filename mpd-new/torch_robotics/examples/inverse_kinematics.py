...
from torch_robotics.visualizers.plot_utils import (
    create_fig_and_axes,
...
if __name__ == "__main__":
    ...
    rot_target = (
        z_rot(-torch.tensor(torch.pi / 2)) @ y_rot(-torch.tensor(torch.pi))
    ).to(**tensor_args)
    frame_target = Frame(rot=rot_target, trans=pos_target, device=device)
    H_target = (
        frame_target.get_transform_matrix()
    )  # set translation and orientation of target here
    ...
    plot_coordinate_frame(
        ax, frame_origin, arrow_length=0.1, arrow_linewidth=3.0, tensor_args=tensor_args
    )
    ...
    for q in q_ik[idx_valid]:
        skeleton = get_skeleton_from_model(
            diff_panda, q, diff_panda.get_link_names()
        )  # visualize IK solution
        skeleton.draw_skeleton(ax=ax, color="blue")

        # draw EE frame
        H_EE = diff_panda.compute_forward_kinematics_all_links(
            q.unsqueeze(0), link_list=["ee_link"]
        )
        frame_EE = Frame(
        ...
        plot_coordinate_frame(
            ax, frame_EE, arrow_length=0.1, arrow_alpha=0.5, tensor_args=tensor_args
        )
