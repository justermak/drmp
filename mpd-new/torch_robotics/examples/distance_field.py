...
def df_obj_list_fn():
    ...
    theta = np.deg2rad(45)
    obj_field_1.set_position_orientation(
        pos=[-1.5, 0.0, 1.0], ori=[np.cos(theta / 2), 0, 0, np.sin(theta / 2)]
    )

    obj_field_2 = ObjectField([sphere])
    theta = np.deg2rad(30)
    obj_field_2.set_position_orientation(
        pos=[1.0, 0.0, 1.0], ori=[np.cos(theta / 2), 0, 0, np.sin(theta / 2)]
    )
...
with TimerCUDA() as t:
    ...
    grad_distance_wrt_q = jacobian(surrogate_compute_distances_objects, q_new).movedim(
        -2, 0
    )
