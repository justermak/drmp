...
class MLP(nn.Module):
    def __init__(
        self, in_dim, out_dim, hidden_dim=16, n_layers=1, act="relu", batch_norm=False
    ):
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = map(
            lambda t: einops.rearrange(t, "... (h c) d -> ... h c d", h=self.heads), qkv
        )
        q = q * self.scale
...
class Conv1dBlock(nn.Module):
    ...
    def __init__(
        ...
        self.block = nn.Sequential(
            ...
            Rearrange(
                "... channels n_support_points -> ... channels 1 n_support_points"
            ),
            nn.GroupNorm(n_groups, out_channels),
            Rearrange(
                "... channels 1 n_support_points -> ... channels n_support_points"
            ),
            nn.Mish(),
        )
        ...
        if self.film_modulation:
            ...
            if self.cond_predict_scale:
                embed = einops.rearrange(
                    embed, "... (c d) 1 -> ... c d 1", c=2, d=self.out_dim
                )
                scale = embed[:, 0, ...]
                bias = embed[:, 1, ...]
                h = scale * h + bias
