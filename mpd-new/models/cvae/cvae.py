...
class Decoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        mid_dim = dims[-1]
        self.latent_to_mid = MLP(
            latent_dim, prod(self.in_shape), hidden_dim=mid_dim, n_layers=1, act="relu"
        )
    ...
    def forward(self, z, context):
        ...
        z = einops.rearrange(
            z, "... (c h) -> ... c h", c=self.in_shape[0], h=self.in_shape[1]
        )
        c_emb = context
...
class CVAEModel(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder = Decoder(
            latent_dim=cvae_latent_dim, in_shape=self.encoder.out_shape, **kwargs
        )
    ...
    def forward(self, x, context_d, **kwargs):
        ...
        guassian_params = self.latent_net(
            einops.rearrange(z_encoder, "... h c -> ... (h c)")
        )
        ...
        gaussian_logvar = self.logvar_min + self.softplus(
            self.logvar_max
            - self.softplus(self.logvar_max - gaussian_logvar)
            - self.logvar_min
        )
    ...
    def loss(self, x, input_dict, loss_cvae_kl_weight=1e-1, **kwargs):
        ...
        kl_divergence = 0.5 * torch.sum(
            -1 - guassian_logvar + torch.exp(guassian_logvar) + guassian_mean**2, dim=-1
        )
    ...
    def reconstruction_loss_fn(x_pred, x):
        assert x_pred.ndim >= 3, (
            f"x_pred.ndim={x_pred.ndim}"
        )  # [batch x horizon x state_dim]
    ...
    def run_inference(
        ...
        **kwargs,
        ...
        return x_sample[
            None, ...
        ]  # [1 x n_samples x horizon x state_dim], dummy dimension to match diffusion step
