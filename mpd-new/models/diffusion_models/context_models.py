...
class ContextModelEEPoseGoal(nn.Module):
    def __init__(self, out_dim=64, n_layers=2, act="relu", **kwargs):
        ...
        self.net = MLP(
            self.in_dim, out_dim, hidden_dim=out_dim, n_layers=n_layers, act=act
        )

    def forward(
        self, ee_goal_orientation_normalized, ee_goal_position_normalized, **kwargs
    ):
        pose_repr = torch.cat(
            (ee_goal_orientation_normalized, ee_goal_position_normalized), dim=-1
        )
...
class ContextModelCombined(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.out_dim = out_dim
        self.net = MLP(
            self.in_dim, self.out_dim, hidden_dim=out_dim, n_layers=n_layers, act=act
        )
