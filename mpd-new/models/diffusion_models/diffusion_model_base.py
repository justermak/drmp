...
class GaussianDiffusionModel(nn.Module, ABC):
    ...
    def predict_x_recon(self, x_t, t, context_d):
        ...
        x_recon = self.predict_start_from_noise(
            x_t, t=t, noise=self.model(x_t, t, context_emb)
        )
    ...
    def p_mean_variance(
        self, x, hard_conds, context_d, t, prior_weight_with_guide=1.0, **kwargs
    ):
        context_emb = None
        if self.context_model is not None:
            context_emb = self.context_model(**context_d)
    ...
    def ddim_sample_loop(
        ...
        **sample_kwargs,
        ...
        sampling_timesteps = (
            ...
            if ddim_sampling_timesteps is not None
        ...
        for k_step, (_time, _time_next) in enumerate(time_pairs):
            ...
            if (
                guide is not None
                and (ddim_sampling_timesteps - k_step) <= t_start_guide
            ):
                with TimerCUDA() as t_guide:
                    ...
                    for k_gd in range(n_guide_steps):
                        ...
                        if compute_costs_with_xrecon:
                            raise NotImplementedError(
                                "compute_costs_with_xrecon is not implemented"
                            )
                        else:
                        ...
                        if scale_grad_by_one_minus_alpha:
                            ...
                            grad_total = (
                                grad_prior_weighted
                                - (1 - alpha).sqrt() * grad_guide_clipped_weighted
                            )
                        else:
                            grad_total = (
                                grad_prior_weighted - grad_guide_clipped_weighted
                            )

                        x_tmp, x_recon = update_x(x, grad_total)
                        # Clip the perturbation to avoid large changes from x_start_opt
                        x_delta = x_tmp - x_start
                        x_delta_clipped = torch.clip(
                            x_delta, -max_perturb_x, max_perturb_x
                        )
    ...
    def conditional_sample(
        self, hard_conds, horizon=None, batch_size=1, method="ddpm", **sample_kwargs
    ):
        """
        ...
        if method == "ddim":
            assert self.predict_epsilon, (
                "ddim only works with predict_epsilon=True, because of guidance"
            )
            return self.ddim_sample_loop(shape_x, hard_conds, **sample_kwargs)
        elif method == "ddpm":
            return self.p_sample_loop(
                shape_x, hard_conds, sample_fn=ddpm_sample_fn, **sample_kwargs
            )
        else:
            raise NotImplementedError
    ...
    def run_inference(
        ...
        **diffusion_kwargs,
        ...
        trajs_chain_normalized = einops.rearrange(
            trajs_chain_normalized, "b diffsteps ... -> diffsteps b ..."
        )
    ...
    def warmup(self, shape_x, device="cuda"):
        ...
        if self.context_model is not None:
            context_emb = torch.randn(
                batch_size, self.context_model.out_dim, device=device
            )
